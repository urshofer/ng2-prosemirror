...
var ProsemirrorComponent = /** @class */ (function () {
     ...
     * @param config
     */
    ProsemirrorComponent.prototype.insertCustomNode = function (text, node) {
        node = node || prosemirror_markdown_1.schema.nodes.paragraph;
    ...
    ProsemirrorComponent.prototype.insertCustomMark = function (text, mark) {
        /*mark.exec()
    ...
    ProsemirrorComponent.prototype.insertCustomTag = function (type) {
        ...
        this.instance.dispatch(this.instance.state.tr.replaceSelectionWith(type.create(null, prosemirror_markdown_1.schema.text(type.name))));
        this.instance.focus();
        //alert('added');
        return true;
    };
    ...
    ProsemirrorComponent.prototype.prosemirrorInit = function (config) {
        ...
        var mixins = [
            ["Index", 'indexword', prosemirror_markdown_1.schema.marks.index],
            ["Mark", 'markername', prosemirror_markdown_1.schema.marks.mark],
            ["Reference", 'markername', prosemirror_markdown_1.schema.marks.reference],
            ["Language", 'ngerman', prosemirror_markdown_1.schema.marks.language],
        ];
        mixins.forEach(function (name) { return menu.insertMenu.content.push(new prosemirror_menu_1.MenuItem({
            title: "Insert: " + name[0],
            label: name[0],
            run: function () { _this.insertCustomMark(name[1], name[2]); }
        })); });
        ...
        menu.insertMenu.content.push(new prosemirror_menu_1.MenuItem({
            ...
            run: function () { _this.insertCustomTag(prosemirror_markdown_1.schema.nodes.latex); }
        }));
        menu.insertMenu.content.push(new prosemirror_menu_1.MenuItem({
            title: "Custom Tag: Comment",
            label: "Custom Tag: Comment",
            run: function () { _this.insertCustomTag(prosemirror_markdown_1.schema.nodes.comment); }
        }));
        menu.insertMenu.content.push(new prosemirror_menu_1.MenuItem({
            title: "Custom Tag: Alternate Paragraph",
            label: "Custom Tag: Alternate Paragraph",
            run: function () { _this.insertCustomTag(prosemirror_markdown_1.schema.nodes.paragraphalternate); }
        }));
